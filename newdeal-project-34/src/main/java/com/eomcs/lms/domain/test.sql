...
CREATE TABLE nd_lesn (
	...
	tot_hr INTEGER      NOT NULL COMMENT '일교육시간', -- 일교육시간
	day_hr INTEGER      NOT NULL COMMENT '총교육시간', -- 총교육시간
...
-- 매니저
ALTER TABLE nd_mger
	ADD CONSTRAINT PK_nd_mger -- 매니저 기본키
		PRIMARY KEY (
			mno -- 매니저번호
		);
...
CREATE TABLE nd_tchr (
	...
	pay_hr INTEGER NOT NULL COMMENT '강의료' -- 강의료
)
COMMENT '강사';
...
-- 수강생
ALTER TABLE nd_student
	ADD CONSTRAINT PK_nd_student -- 수강생 기본키
		PRIMARY KEY (
			mno -- 수강생번호
		);
...
-- 수업
ALTER TABLE nd_lesn
	ADD CONSTRAINT FK_nd_mger_TO_nd_lesn -- 매니저 -> 수업
		FOREIGN KEY (
			mno -- 매니저번호
		)
		REFERENCES nd_mger ( -- 매니저
			mno -- 매니저번호
		);

-- 매니저
ALTER TABLE nd_mger
	ADD CONSTRAINT FK_nd_memb_TO_nd_mger -- 회원 -> 매니저
		FOREIGN KEY (
			mno -- 매니저번호
		)
...
-- 수강신청
ALTER TABLE nd_appl
	ADD CONSTRAINT FK_nd_student_TO_nd_appl -- 수강생 -> 수강신청
		FOREIGN KEY (
			mno -- 수강생번호
		)
		REFERENCES nd_student ( -- 수강생
			mno -- 수강생번호
		);
...
-- 수강생
ALTER TABLE nd_student
	ADD CONSTRAINT FK_nd_addr_TO_nd_student -- 주소 -> 수강생
		FOREIGN KEY (
			ano -- 주소번호
		)
...
-- 수강생
ALTER TABLE nd_student
	ADD CONSTRAINT FK_nd_memb_TO_nd_student -- 회원 -> 수강생
		FOREIGN KEY (
			mno -- 수강생번호
		)
		REFERENCES nd_memb ( -- 회원
